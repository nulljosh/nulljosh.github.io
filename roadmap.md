...
- [x] Portfolio iOS app should also show the Animoji — added as a `Memoji` imageset, rendered at 40pt in the header (40pt is the largest size the 128px source stays crisp at, even @3x). Build verified.
- [x] Journal/portfolio out of sync — ROOT CAUSE: `journal/scripts/sync-portfolio.sh` rewrote the portfolio's `index.html` but never committed or pushed it, and the portfolio is a separate repo, so every journal deploy left the change stranded in the working tree while the live site drifted. The live site was missing the newest post and still showing a May entry. Script now commits + pushes, the 4-post cap is gone (all 7 now mirror the journal), and the year label attaches to the first post of each year instead of only the first post overall.
- [x] Reflect the Echo → voxprint rename in portfolio content/links — landing page copy/title/App Store links rewritten, assets moved off `raw.githubusercontent.com/nulljosh/echo` to relative paths, `voxprint.heyitsmejosh.com` attached to the Pages project (DNS already pointed there but Pages did not serve the hostname, hence the 522) and now returns 200. Portfolio and iOS app both link to it.
