...
- [ ] Unused legacy avatar assets: `images/memoji.png` (6535 opaque near-white px — hard white background, no alpha edge) and the old `images/memoji-face.png` are both unreferenced now. Delete once the fix above is confirmed on-device, so neither can be reintroduced by mistake.
